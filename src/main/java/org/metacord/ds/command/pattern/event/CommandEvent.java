package org.metacord.ds.command.pattern.event;

import org.metacord.ds.command.pattern.meta.Command;
import org.metacord.ds.command.pattern.meta.Help;
    ...
    public void onMessageCreate(MessageCreateEvent event) {
        ...
        try {
            if(containsKeyIgnoringCase(helpCommands, command.replace(command.substring(0, 1), "0"))){
                ...
                Method m = c.getMethod("help", CommandData.class);
                Object o = c.getDeclaredConstructor().newInstance();
                m.invoke(o, new CommandData<>(
                        (CommandListener) o,
                        event
                ));
                return;
        ...
        try {
            if(containsKeyIgnoringCase(commands, headCommand.replace(headCommand.substring(0, 1), "0"))){
                ...
                Method m = c.getMethod("onCommand", CommandData.class);
                Object o = c.getDeclaredConstructor().newInstance();
                m.invoke(o, new CommandData<>(
                        (CommandListener) o,
                        event
